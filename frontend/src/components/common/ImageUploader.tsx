...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const imageClasses = shape === 'circle' 
    ...
    : 'w-full max-w-xs object-cover rounded-lg border-2 border-gray-300';
  ...
  return (
    <div className="w-full">
      ...
      {isCompressing ? (
        <div className={`w-full ${height} border-2 border-dashed rounded-lg flex items-center justify-center bg-gray-50`}>
        ...
        <div className={shape === 'circle' ? 'flex items-center space-x-4' : 'space-y-2'}>
          ...
          <button type="button" onClick={handleClick} className="text-sm text-green-600 hover:text-green-700">
        ...
        <div
          ...
          onDrop={handleDrop}
          onClick={handleClick}
          className={`w-full ${height} border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer transition-colors ${isDragging ? 'border-green-500 bg-green-50' : 'border-gray-300 hover:border-gray-400'}`}
        >
          ...
          <p className="text-gray-400 text-xs">
            {t('imageUploader.formats', { formats: 'JPEG, PNG' })} / {t('imageUploader.maxSize', { maxSize: maxSizeMB })}
          </p>
        </div>
      )}

      <input ref={fileInputRef} type="file" accept={allowedFormats.join(',')} onChange={handleFileChange} className="hidden" />
