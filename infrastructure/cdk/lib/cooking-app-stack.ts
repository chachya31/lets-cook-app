...
export class CookingAppStack extends cdk.Stack {
  ...
  private createLambdaFunction(stage: string): lambda.IFunction {
    // Gemini APIキーは Secrets Manager コスト削減のため Lambda 環境変数で直接渡す
    const geminiApiKey = process.env.GEMINI_API_KEY;
    if (!geminiApiKey) {
      throw new Error(
        'GEMINI_API_KEY environment variable is required for cdk deploy. ' +
          'Set it before running deploy: e.g. `GEMINI_API_KEY=xxxx cdk deploy CookingAppStack-Dev`'
      );
    }
