...
- **レスポンシブデザイン**: **Desktop First (PC Base)**
  - PC画面（デスクトップ）でのレイアウトとUXを最優先に設計する
  - モバイル画面（320px〜）では、グリッドの縦積み(stacking)や横スクロールの回避などを行い、「レイアウトが崩れず、自然に閲覧・操作できる」状態を保証すること（Graceful Degradation）
  - 無理にモバイル専用のUIを作り込む必要はないが、表示崩れはNGとする
- **テーマ**:
  - ベース: Zinc (ニュートラル)
